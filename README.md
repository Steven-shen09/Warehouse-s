...
全栈物品仓库管理系统，支持多仓库管理、仓库级借出追踪、库存调拨、物品盘点、租借审批、归还追踪、批量导入导出和 RBAC 权限控制。
...
|------|------|
| 后端 | Python FastAPI |
| 数据库 | PostgreSQL 16 + SQLAlchemy Core |
| 前端 | Jinja2 + HTMX 2.0 + Alpine.js 3.14 |
| 样式 | Cursor Warm Minimal 设计系统（亮色/暗色模式） |
...
> 可使用 `docker-compose up -d` 快速启动 PostgreSQL 容器。
...
### 核心业务
- **多仓库管理** — 5 大仓库独立管理，物品分仓库存，仓库级借出/剩余量实时展示
- **仓库级追踪** — 每笔借出精确记录来源仓库，按仓库展示借出量/剩余量，借出时自动校验单仓库存
- **库存调拨** — 批量创建调拨单，源仓库库存校验，审核通过后自动转移库存
- **物品盘点** — 创建盘点计划，逐行录入或批量导入实盘数据，差异自动更新库存
- **物品管理** — CRUD、分类/仓库筛选、分仓库存展开（库存/借出/剩余）、低库存预警
- **租借引擎** — 批量租借（购物车模式）、仓库级库存检查、事务保护、部分归还
